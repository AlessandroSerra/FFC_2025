...
def boundCond(T_old, T_new, bc):
    if bc == "d":
        ...
        raise ValueError(
            "La condizione deve essere di dirichlet (d) oppure neumann (n)"
        )
...
def stepFTCS(T_old, N_x, dt, k, h, bc):
    ...
    for i in range(1, N_x - 1):
        T_new[i] = T_old[i] + dt * k / h**2 * (
            T_old[i - 1] + T_old[i + 1] - 2 * T_old[i]
        )
...
def Solve(T0, T1, N_x, N_t, dt, k, h, bc, tresh):
    ...
    for i in range(1, N_t):
        ...
        deltaT = np.abs(T[i, 1:-1] - T[i - 1, 1:-1])
        if np.max(deltaT) < tresh:
            print(f"Convergenza raggiunta al tempo {(i+1) * dt:.2f} s (iterazione {i})")
            T[i:] = T[i]  # manteniamo il profilo costante dopo la convergenza
            break
...
def fitAmps(T, N_t, dt):
    ...
    popt, pcov = curve_fit(
        ...
        times,
        max_Ts,
        p0=[np.max(max_Ts), (N_t // 2) * dt, np.min(max_Ts)],
    )
...
T = Solve(T0, T1, N_x, N_t, dt, k, h, bc="n", tresh=thresh)
...
ax.plot(
    times,
    y_fit,
    label=f"Fit: $\\tau$ = ({tau:.0f}$\\pm${err_tau:.0f}) s",
    color="red",
)  # :.1e mette notazione scientifica con 1 cifra significativa
ax.set_xlabel("t [s]")
ax.set_ylabel("T [K]")
